...
const SCORING_ROWS = [
  { label: '>12,500 steps/day',    sub: 'EXCELLENT',     pts: 100, dot: '#059669' },
  { label: '7,500–12,499 steps',   sub: 'GOOD',          pts: 75,  dot: '#3B82F6' },
  { label: '4,000–7,499 steps',    sub: 'FAIR',          pts: 50,  dot: '#F59E0B' },
  { label: '<4,000 steps/day',     sub: 'POOR',          pts: 25,  dot: '#DC2626' },
];
...
const PhysicalActivityLandingScreen: React.FC = () => {
  ...
  const fetchTodayData = useCallback(() => {
    ...
    fetch(url, { headers: { Authorization: `Bearer ${accessToken}` } })
      .then(r => r.json())
      .then(data => {
        if (data.steps != null) setTodaySteps(Number(data.steps));
        if (data.score != null) setTodayScore(Number(data.score));
        if (data.weeklyAvg != null) setWeeklyAvgSteps(Number(data.weeklyAvg));
      })
  ...
  const fetchChart = useCallback((period: 'week' | 'month' | 'year') => {
    ...
    fetch(`${HEALTH_SCORES_BASE}/physical-activity/history?period=${period}&timezone=${encodeURIComponent(tz)}`, {
  ...
  const trendLabel = (() => {
    if (todaySteps === null || weeklyAvgSteps === null) return null;
    if (todaySteps > weeklyAvgSteps) return { arrow: '↑', text: 'Above 7-day average' };
    if (todaySteps < weeklyAvgSteps) return { arrow: '↓', text: 'Below 7-day average' };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.measureCard}>
          ...
          <Text style={styles.measureBody}>
            Your Physical Activity score is measured by your daily step count, synced automatically from your Fitbit device. Steps are used as a proxy for overall physical activity levels.
          </Text>
        </View>
        ...
        <View style={styles.statsCard}>
            ...
            <View style={styles.statLeft}>
              ...
              <Text style={styles.statSub}>
                {!fitbitConnected
                  ...
                  : todaySteps === null
                    ? 'No steps data for today'
                    : `Score: ${todayScore ?? '—'} pts`}
              </Text>
            </View>
            <View style={styles.statPill}>
              <Text style={styles.statPillText}>
                {todaySteps !== null ? (todaySteps >= 1000 ? `${(todaySteps / 1000).toFixed(1)}k` : String(todaySteps)) : '—'}
              </Text>
            </View>
          </View>
          ...
          <View style={[styles.statRow, { marginTop: 12 }]}>
            <View style={styles.statLeft}>
              <Text style={styles.statTitle}>7-Day Avg Steps</Text>
              {trendLabel ? (
            ...
            <View style={styles.statPill}>
              <Text style={styles.statPillText}>
                {weeklyAvgSteps !== null
                  ? (weeklyAvgSteps >= 1000 ? `${(weeklyAvgSteps / 1000).toFixed(1)}k` : String(weeklyAvgSteps))
                  : '—'}
              </Text>
            </View>
          </View>
        ...
        <Text style={styles.footnote}>Step count is synced automatically from your Fitbit device. Make sure your device is connected and synced for accurate data.</Text>
...
export default PhysicalActivityLandingScreen;
