...
-- (a circular foreign key). Checks are re-enabled at the very end of this file.
...
CREATE TABLE IF NOT EXISTS ema_enrollments (
  ...
  FOREIGN KEY (user_id) REFERENCES user_auth_testing(id) ON DELETE CASCADE,
  -- SET NULL: deleting a referenced event/response orphans the enrollment
  -- rather than deleting it (matches the "null for weekly/momentary" design).
  FOREIGN KEY (symptom_event_id) REFERENCES symptom_events(id) ON DELETE SET NULL,
  FOREIGN KEY (instrument_response_id) REFERENCES symptom_instrument_responses(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS symptom_instrument_responses (
  ...
  FOREIGN KEY (patient_id) REFERENCES user_auth_testing(id) ON DELETE CASCADE,
  -- SET NULL: deleting an enrollment/plan detaches the response rather than
  -- deleting it (matches the "null if patient has not enrolled" design).
  FOREIGN KEY (enrollment_id) REFERENCES ema_enrollments(id) ON DELETE SET NULL,
  FOREIGN KEY (weekly_plan_id) REFERENCES weekly_symptom_plans(id) ON DELETE SET NULL,
...
-- 7. LEGACY (removed)
-- =============================================================================
-- The dead `user_goals` table (replaced by the live activity_goals /
-- daily_goals flow) is not created here. full_migrate.sql retains a DROP for
-- it to clean up pre-existing copies; the additive variant has no DROPs.
